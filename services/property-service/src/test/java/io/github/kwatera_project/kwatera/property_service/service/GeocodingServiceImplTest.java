...
class GeocodingServiceImplTest {
    ...
    void getCoordinates_ShouldReturnCoordinates_WhenAddressIsValid() throws JsonProcessingException {
        ...
        mockServer.expect(requestTo("https://nominatim.openstreetmap.org/search?q="
                        + expectedQuery.replace(" ", "%20").replace(",", "%2C")
                        + "&format=jsonv2"))
                .andExpect(header("User-Agent", "kwatera-app"))
                .andRespond(withSuccess(jsonResponse, MediaType.APPLICATION_JSON));

        // When
        Coordinates result = geocodingService.getCoordinates(street, streetNumber, postalCode, city, country);
    ...
    @Test
    void getCoordinates_ShouldThrowResponseStatusException_WhenResponseIsEmpty() throws JsonProcessingException {
        // Given
        ...
        // When & Then
        assertThatThrownBy(() -> geocodingService.getCoordinates(street, streetNumber, postalCode, city, country))
