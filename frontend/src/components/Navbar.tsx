...
export default function Navbar({ isSubpage }: NavbarProps = {}) {
    ...
    const isScrolledActive = isScrolled || isEffectiveSubpage;

    const positionClass = isEffectiveSubpage ? 'sticky top-0 mb-8' : 'fixed top-0';
        ...
        <nav className={`${positionClass} w-full z-[1000] transition-all duration-300 ease-in-out ${bgAndPaddingClass}`}>
            <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 flex justify-between items-center relative">
                ...
                <div className="hidden lg:flex items-center gap-8 absolute left-1/2 -translate-x-1/2">
                    {navLinks.map((link) => (
                        <Link
                            key={link.name}
                            to={link.path}
                            className={`font-medium text-sm tracking-widest uppercase transition-all duration-300 ${isScrolledActive ? 'text-stone-800 hover:text-[#8B4513]' : 'text-white hover:text-white/80 drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]'}`}
                        >
                            {link.name}
                        </Link>
                    ))}
                ...
                <div className="flex items-center gap-6 md:gap-8">
                    ...
                    <div className={`h-5 w-px hidden sm:block transition-all duration-300 ${isScrolledActive ? 'bg-stone-300' : 'bg-white/20'}`}></div>

                    {isLoggedIn ? (
                        <div className="relative" ref={profileDropdownRef}>
                            <button
                                onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)}
                                className={`flex items-center gap-2 rounded-full border pl-1.5 pr-4 py-1.5 transition-all duration-300 focus:outline-none ${isScrolledActive ? 'bg-transparent border-stone-300 text-stone-800 hover:bg-stone-100/50' : 'bg-transparent border-white/30 text-white hover:bg-white/10'}`}
                            >
                                <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-300 ${isScrolledActive ? 'bg-stone-100 text-stone-800 border border-stone-200' : 'bg-white/15 text-white'}`}>
                                    <User size={16} />
                                </div>
                                <span className={`font-semibold text-xs tracking-wider uppercase hidden md:block max-w-[100px] truncate transition-all duration-300 ${isScrolledActive ? '' : 'drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]'}`}>{displayName}</span>
                            </button>

                            {isProfileDropdownOpen && (
                                <div className="absolute top-full right-0 mt-3 w-56 bg-card rounded-2xl shadow-2xl z-[1000] overflow-hidden border border-[#DACDCA]/40 py-2 animate-in fade-in slide-in-from-top-2 duration-200">
                                    <div className="px-4 py-3 border-b border-gray-100 mb-1">
                                        <p className="text-xs font-bold uppercase tracking-wider text-details">My Account</p>
                                    </div>

                                    <Link to="/profile" className="flex items-center gap-3 px-4 py-2.5 text-sm text-[rgb(var(--color-burgundy))] hover:bg-gray-50 transition-colors font-semibold">
                                        <Settings size={16} />
                                        My Profile
                                    </Link>

                                    {(!userRoles.includes("ROLE_ADMIN") && !userRoles.includes("ROLE_OWNER")) && (
                                        <Link to="/my-reservations" className="flex items-center gap-3 px-4 py-2.5 text-sm text-[rgb(var(--color-burgundy))] hover:bg-gray-50 transition-colors font-semibold">
                                            <Calendar size={16} />
                                            My Reservations
                                        </Link>
                                    )}
                                    ...
                                    {(userRoles.includes("ROLE_ADMIN") || userRoles.includes("ROLE_OWNER")) && (
                                        <>
                                            <Link to="/admin/reservations" className="flex items-center gap-3 px-4 py-2.5 text-sm text-[rgb(var(--color-burgundy))] hover:bg-gray-50 transition-colors font-semibold">
                                                <Calendar size={16} />
                                                Reservations
                                            </Link>
                                            <Link to="/admin/dashboard" className="flex items-center gap-3 px-4 py-2.5 text-sm text-[rgb(var(--color-burgundy))] hover:bg-gray-50 transition-colors font-semibold">
