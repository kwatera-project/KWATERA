...
export default function Navbar({isSubpage}: NavbarProps = {}) {
    ...
    return (
        <nav
            className={`${positionClass} w-full z-[1000] transition-all duration-300 ease-in-out ${bgAndPaddingClass}`}>
            <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 flex justify-between items-center relative">
                ...
                <Link to="/" className="flex items-center gap-2.5 group leading-none">
                    ...
                    <span
                        className={`font-normal text-2xl tracking-widest hidden sm:block self-center transition-all duration-300 ${isScrolledActive ? 'text-stone-800' : 'text-white drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]'}`}>KWATERA</span>
                </Link>
                ...
                <div className="flex items-center gap-6 md:gap-8">
                    ...
                    {isLoggedIn ? (
                        <div className="relative" ref={profileDropdownRef}>
                            ...
                            >
                                <div
                                    className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-300 ${isScrolledActive ? 'bg-stone-100 text-stone-800 border border-stone-200' : 'bg-white/15 text-white'}`}>
                                    <User size={16}/>
                                </div>
                                <span
                                    className={`font-semibold text-xs tracking-wider uppercase hidden md:block max-w-[100px] truncate transition-all duration-300 ${isScrolledActive ? '' : 'drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]'}`}>{displayName}</span>
                            </button>
